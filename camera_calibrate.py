...
def crop(image):
    y_max, x_max, depth = image.shape
    print(x_max, y_max)
    return image[125:y_max-125,225:x_max-225].copy()

cam1 = cv2.VideoCapture(0)
cam1.set(cv2.CAP_PROP_FRAME_WIDTH, 100000)
cam1.set(cv2.CAP_PROP_FRAME_HEIGHT, 1000000)
...
while len(images) < 200:
    ...
    if ret == True:
        ...
        frame_w_corners = cv2.drawChessboardCorners(frame_w_corners, CHECKERBOARD, corners, ret)
...
for img in images:
    ...
    if ret == True:
        ...
        img_w_corners = cv2.drawChessboardCorners(img_w_corners, CHECKERBOARD, corners, ret)
